...
func (q *priorityQueue) Swap(i, j int) {
	...
	q.lookups[q.items[i].s.Hash()] = i
	q.lookups[q.items[j].s.Hash()] = j
}

func (q *priorityQueue) Push(x interface{}) {
	n := len(q.items)
	item := x.(pqItem)
	q.lookups[item.s.Hash()] = n
...
func (q *priorityQueue) remove(s State) {
	index := q.lookups[s.Hash()]
	delete(q.lookups, s.Hash())
	heap.Remove(q, index)
}

func newPriorityQueue() *priorityQueue {
	q := new(priorityQueue)
	q.lookups = make(map[string]int)
